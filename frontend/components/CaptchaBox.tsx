...
export default function CaptchaBox({ disabled = false, onChange, purpose }: CaptchaBoxProps) {
  ...
  return (
    <div className="smooth-card rounded-lg border border-black/10 bg-white/70 p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        <label className="label !mb-0" htmlFor={`${purpose}-captcha`}>
          Security check
        </label>
        <button className="btn-secondary scale-tap !px-2 !py-2" disabled={disabled || loading} onClick={loadChallenge} title="Refresh security check" type="button">
          <RefreshCw className={loading ? "animate-spin" : ""} size={16} />
        </button>
      </div>
      {error ? (
        <p className="fade-in-up text-sm font-bold text-rose-700">{error}</p>
      ) : challenge?.image_base64 ? (
        <div className="motion-safe-soft mb-3 rounded-lg border border-black/10 bg-[#fbfaf7] p-2">
          <img alt="Security check image" className="h-20 w-full object-contain" draggable={false} src={challenge.image_base64} />
        </div>
      ) : (
        <p className="loading-soft mb-3 text-sm font-bold text-[#526069]">Connecting to the server. This may take a few seconds.</p>
      )}
